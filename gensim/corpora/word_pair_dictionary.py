...
class BigramDictionary(utils.SaveLoad, Mapping):
    def __init__(self, documents=None, prune_at=2000000, no_below=5, no_above=0.5,
                 bigram=True, bigram_window=True, bigram_window_size=10):
        self.token2id = {}
        self.id2token = {}
        self.dfs = {}
        ...
        self.bigram = bigram
        self.bigram_window = bigram_window
        self.bigram_window_size = bigram_window_size

        self.num_docs = 0
        # Total number of corpus positions (number of processed words).
        self.num_pos = 0
        # Total number of non-zeroes in the BOW matrix (sum of the number of unique words per document over the entire
        # corpus).
        self.num_nnz = 0
    ...
    def from_documents(documents):
        ...
        """
        return BigramDictionary(documents=documents)

    def add_documents(self, documents, prune_at=2000000):
        print('Processing unigram tokens')
        tic = time.time()
        for docno, document in enumerate(documents):
            # log progress & run a regular check for pruning, once every 10k docs
            if docno > 0 and (docno + 1) % 10000 == 0:
                # if prune_at is not None and len(self) > prune_at:
                #     self.filter_extremes(no_below=0, no_above=1.0, keep_n=prune_at)
                logger.info("adding document #%i to %s", docno, self)
                toc = time.time()
                print('doc2bow {} documents in {} '.format(docno + 1, str(dt.timedelta(seconds=toc - tic))))

            # update Dictionary with the document
            self.doc2bow(document, allow_update=True)  # ignore the result, here we only care about updating token ids
        self.filter_extremes(no_below=self.no_below, no_above=self.no_above)
        print("Dictionary of unigram size is {}".format(len(self.dfs)))

        if self.bigram:
            print('Processing bigram tokens')
            for docno, document in enumerate(documents):
                # log progress & run a regular check for pruning, once every 10k docs
                if docno > 0 and (docno + 1) % 10000 == 0:
                    # if prune_at is not None and len(self) > prune_at:
                    ...
                    logger.info("adding document #%i to %s", docno, self)
                    toc = time.time()
                    print('doc2bigram {} documents in {} '.format(docno + 1, str(dt.timedelta(seconds=toc - tic))))

                # update Dictionary with the document
                self.doc2bigram(document, allow_update=True)  # ignore the result, here we only care about updating token ids
            self.filter_extremes(no_below=self.no_below, no_above=self.no_above)
        print("Dictionary of unigram and bigram size is {}".format(len(self.dfs)))

    def doc2bigram(self, document, allow_update=False, return_missing=False):
        ...
        len_temp = len(document) - 1
        for i, w1 in enumerate(document[:-1]):
            window_temp = self.bigram_window * (i + self.bigram_window_size) + (1 - self.bigram_window) * len(document)
            for j, w2 in enumerate(document[i+1:min(len_temp, window_temp)]):
                if w1 == w2:
                    continue
                # sort two words alphabetically. loan payment and payment loan are both converted to 'loan_payment'
                word_pair = min(w1, w2) + '_' + max(w1, w2)
                counter[word_pair] += 1
    ...
    def doc2bow(self, document, allow_update=False, return_missing=False):
        ...
        # Count unigram word frequency
        for w in document:
            counter[w if isinstance(w, unicode) else unicode(w, 'utf-8')] += 1

        # Create a token2id mapping
        token2id = self.token2id
        if allow_update or return_missing:
            # missing is a list of tuples [('account', 1), ('client', 1), ... ]
            missing = sorted(x for x in iteritems(counter) if x[0] not in token2id)
            if allow_update:
                for w, _ in missing:
                    # new id = number of ids made so far;
                    # NOTE this assumes there are no gaps in the id sequence!
                    token2id[w] = len(token2id)
        # result is a mapping from tokenid to frequency in this document
        result = {token2id[w]: freq for w, freq in iteritems(counter) if w in token2id}
        # Calculate id2token automatically
        self.id2token = utils.revdict(token2id)
